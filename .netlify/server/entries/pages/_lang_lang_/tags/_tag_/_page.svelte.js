...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  ready = !!data.tagname.letters.length;
  $$unsubscribe_page();
  return ` ${$$result.head += `<!-- HEAD_svelte-1qe6hp6_START -->${data.tagname.letters.length ? `${$$result.title = `<title>${escape(t.theme)} : ${escape(data.tagname.tag)}</title>`, ""}` : ``}<!-- HEAD_svelte-1qe6hp6_END -->`, ""} <article>${ready ? ` <div><h2>${escape(t.theme)} : ${escape(data.tagname.tag)}</h2> <ul>${each(data.tagname.letters, (lettre) => {
